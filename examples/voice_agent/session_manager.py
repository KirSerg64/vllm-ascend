...
A **stable** ``llm_continuation_id`` is assigned once per session and reused
as the ``request_id`` for every ``engine.generate()`` call.  This allows
vLLM's AsyncLLM to retain the KV-cache across turns so the full conversation
history never needs to be re-fed to the engine — only the delta for each new
turn is submitted.
...
class SessionState:
    ...
    # -----------------------------------------------------------------
    # LLM continuation state
    # -----------------------------------------------------------------
    # Stable request_id reused across all turns for the same session so
    # that vLLM's AsyncLLM retains the KV-cache between turns.
    # Only the delta (previous assistant turn + new user turn) is fed each
    # time; the engine appends to its cached prefix automatically.
    llm_continuation_id: str = field(default_factory=lambda: f"llm-{uuid.uuid4().hex}")
    ...
    def last_assistant_response(self) -> str | None:
        """Return the most recent assistant turn content, or None."""
